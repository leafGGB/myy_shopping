...
// 工具函数
import { serilalize, addURLData, serilalizeJSON } from './utils.js';
...
class Ajax {
    constructor(url, options) {
        // 希望其他方法也能访问到 url ，如果不保存到 this 中，那么 url 只是这个构造方法中的全局变量
        this.url = url;
    ...
    addParam() {
        ...
        return addURLData(this.url, serilalize(params));
    }
    ...
    sendData() {
        ...
        if (this.isFormData()) {
            ...
            resultData = serilalize(data);
        } else if (this.isJSONData()) {
            // 发送 application/json 格式的数据
            this.setContentType(CONTENT_TYPE_JSON);
            resultData = serilalizeJSON(data);
        } else {
            // 发送其他格式的数据
            this.setContentType();
            resultData = data;
